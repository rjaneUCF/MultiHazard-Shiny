...
fluidPage(
  navbarPage(strong("MulitHazard"), id="bfa",
             tabPanel("Data",
                      id="data",
                      # Application title
                      titlePanel("Data"),
  ...
  tabPanel("Detrend",
           ...
           titlePanel("Detrending the time series"),
              tabsetPanel(id = "Detrend_tabs",
                       tabPanel(title = textOutput("var1_name_detrend_text"),
                                id = "detrend_tab_1",
                                value = "detrend_var_1",
                                sidebarLayout(
                                  sidebarPanel(
                                    ...
                                    conditionalPanel("input.detvar1=='yes'",
                                                     tagList(
                                                       h4(strong("Method")),
                                                       helpText("Please choose the detrending method"),
                                                       selectInput("detrendmethodvar1", NULL,
                                                                   c(Linear = "linear",Window = "window"))
                                                     )),
                                          conditionalPanel("input.detrendmethodvar1=='window'",
                       ...
                       tabPanel(title = textOutput("var2_name_detrend_text"),
                                value = "detrend_var_2",
                                sidebarLayout(
                                  sidebarPanel(
                                    h4(strong("Detrend")),
                                    ...
                                    selectInput("detvar2", NULL,
                                                c(Yes = "yes", No = "no"), selected="yes"),
                                    # select plot type
                                        conditionalPanel("input.detvar2=='yes'",
                                                     tagList(
                                                      h4(strong("Method")),
                                                      helpText("Please choose the detrending method"),
                                                      selectInput("detrendmethodvar2",NULL,
                                                                   c(Linear = "linear", Window = "window"))
                                                     ),
                                      conditionalPanel("input.detrendmethodvar2=='window'",
                                        h4(strong("Window width")),
                                        helpText("Please choose width of moving window"),
                                        numericInput("detrendwindowwidthvar2", NULL,10)
                                    ))
           ...
           tabPanel("Decluster",
                    ...
                    titlePanel("Declustering the time series"),
                      tabsetPanel(id = "Decluster_tabs",
                                 tabPanel(title = textOutput("var1_name_decl_text"),
                                          value = "decluster_var_1",
                                
                        sidebarPanel(
                                  h4(strong("Method")),
                                  helpText("Please choose the declustering method"),
                                  selectInput("decvar1", NULL, selected="",
                                              c(Window = "Solari", Runs = "Standard")
                                   ),
                              
                             conditionalPanel(condition = "input.decvar1 == 'Solari'",
                                                  tagList(
                                                    h4(strong("Window width")),
                                                    helpText("Please choose declustering window width"),
                                                    numericInput("decwindowwidthvar1", NULL,"10"))
                                              ),
                              conditionalPanel(condition = "input.decvar1 == 'Standard'",
                                                  tagList(
                                                    h4(strong("Threshold")),
                     ...
                     tabPanel(title = textOutput("var2_name_decl_text"),
                              value = "decluster_var_2",
                              
                              sidebarPanel(
                                h4(strong("Method")),
                                helpText("Please choose the declustering method"),
                                selectInput("decvar2", NULL, selected= "",
                                            c(Window = "Solari", Runs = "Standard")
                                ),
                                
                                conditionalPanel(condition = "input.decvar2 == 'Solari'",
                                                 tagList(
                                                   h4(strong("Window width")),
                                                   helpText("Please choose declustering window width"),
                                                   numericInput("decwindowwidthvar2", NULL,"10"))
                                ),
                                conditionalPanel(condition = "input.decvar2 == 'Standard'",
                                                 tagList(
                                                   h4(strong("Threshold")),
                                                   helpText("Please choose the quantile threshold"),
  ...
  tabPanel("Dependence structure",
           tabsetPanel(
             id = "Sample_conditioned_on_rainfall",
             tabPanel(title = textOutput("sample_conditioned_on_var1_dep_str"),
                      value = "copula_tab_1",
                      sidebarLayout(
                        sidebarPanel(
             ...
             tabPanel(title = textOutput("sample_conditioned_on_var2_dep_str"),
                      value = "copula_tab_2",
                      sidebarLayout(
                        sidebarPanel(
                          h4(strong("Quantile threshold")),
  ...
  tabPanel("Marginal distributions",
           tabsetPanel(id = "Conditioned_samples",
                       tabPanel(textOutput("sample_conditioned_on_var1_mar_dist"),
                                tabsetPanel(id = "Sample_conditioned_on_var_1", 
                                            tabPanel(title = textOutput("gpd_var1_text"),
                                                     value = "extreme_var1",
                                                     sidebarLayout(
                                                       sidebarPanel(
                                                         h4(strong("Method")),
                                            ...
                                            tabPanel(title = textOutput("non_extreme_var2_text"),
                       ...
                       tabPanel(textOutput("sample_conditioned_on_var2_mar_dist"),
                                ...
                                tabsetPanel(id = "Sample_conditioned_on_var2",
                                            tabPanel(title = textOutput("gpd_var2_text"),
                                                     sidebarPanel(
                                                       h4(strong("Method")),
                                                       helpText("Please specify the method used to decluster the time series"),
                                            ...
                                            tabPanel(title = textOutput("non_extreme_var1_text"),
  ...
  tabPanel("Isolines",
           sidebarLayout(
                      sidebarPanel(h3(textOutput("sample_conditioned_on_var1_iso")),
